(function($) {
	...
	function startTimeliner(options) {
		...
		if ($(settings.timelineContainer).data("started")) {
			...
			$(settings.timelineContainer + " " + ".expandAll").html(
					settings.expandAllText);
			$(settings.timelineContainer + " " + ".collapseAll").html(
					settings.collapseAllText);
			if (settings.startState === "closed") {
				$(settings.timelineContainer + " " + ".timelineEvent").hide();
				$.each($(settings.startOpen), function(index, value) {
					openEvent($(value)
							.parent(
									settings.timelineContainer + " "
											+ ".timelineMinor").find("dt a"),
							$(value))
				})
			} else {
				openEvent($(settings.timelineContainer + " "
						+ ".timelineMinor dt a"), $(settings.timelineContainer
						+ " " + ".timelineEvent"))
			}
			...
			$(settings.timelineContainer).on(
					"click",
					".timelineMajorMarker",
					function() {
						var numEvents = $(this).parents(".timelineMajor").find(
								".timelineMinor").length;
						var numOpen = $(this).parents(".timelineMajor").find(
								".open").length;
						if (numEvents > numOpen) {
							openEvent($(this).parents(".timelineMajor").find(
									"dt a", "dl.timelineMinor"), $(this)
									.parents(".timelineMajor").find(
											".timelineEvent"))
						} else {
							closeEvent($(this).parents(".timelineMajor").find(
									"dl.timelineMinor a"), $(this).parents(
									".timelineMajor").find(".timelineEvent"))
						}
					});
			$(settings.timelineContainer + " " + ".expandAll").click(
					function() {
						if ($(this).hasClass("expanded")) {
							closeEvent($(this).parents(
									settings.timelineContainer).find("dt a",
									"dl.timelineMinor"), $(this).parents(
									settings.timelineContainer).find(
									".timelineEvent"));
							$(this).removeClass("expanded").html(
									settings.expandAllText)
						} else {
							openEvent($(this).parents(
									settings.timelineContainer).find("dt a",
									"dl.timelineMinor"), $(this).parents(
									settings.timelineContainer).find(
									".timelineEvent"));
